...
export const apiSwapBaseOut = async () => {
  const owner: Keypair = Keypair.fromSecretKey(bs58.decode(""))
  ...
  if (!isV0Tx) {
    for (const tx of allTransactions) {
      ...
      const txId = await sendAndConfirmTransaction(solanaWalletUtils.getConnection(), transaction, [owner], { skipPreflight: true })
    ...
    for (const tx of allTransactions) {
      ...
      transaction.sign([owner])
      const txId = await solanaWalletUtils.getConnection().sendTransaction(tx as VersionedTransaction, { skipPreflight: true })
      const { lastValidBlockHeight, blockhash } = await solanaWalletUtils.getConnection().getLatestBlockhash({
        commitment: 'finalized',
      })
      console.log(`${idx} transaction sending..., txId: ${txId}`)
      await solanaWalletUtils.getConnection().confirmTransaction(
        {
          blockhash,
