...
export const harvestLockedPosition = async (privateKey: string) => {
  const raydium = await initSdk(privateKey, { loadToken: true })
  ...
  const lockData = LockClPositionLayoutV2.decode(res!.data as any)
