...
import {printSimulateInfo} from "../utils/printUtils";
import {NATIVE_MINT} from "@solana/spl-token";

export const swap = async (privateKey: string, poolInfo: ApiV3PoolInfoConcentratedItem, amountIn: number, inputMint: string=NATIVE_MINT.toBase58(), slippage:number = 0.01, microLamports: number) => {
  ...
  const swapData = await raydium.clmm.swap({
    poolInfo,
    poolKeys,
